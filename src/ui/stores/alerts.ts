...
export class AlertsStore extends BaseStore {
  @observable alerts: {
    message: React.Component | React.FunctionComponent | string;
    timestamp: number;
  }[] = [];
  ...
  constructor() {
    ...
    this.cleanAlertsTimer = setInterval(() => {
      ...
      this.alerts = this.alerts.length
        ? this.alerts.filter((alert) => now - alert.timestamp < 150000)
        : this.alerts;
    }, 500);
  }
  ...
  push(message: React.Component | React.FunctionComponent | string) {
    this.alerts.push({
      message,
      timestamp: Date.now(),
    });
